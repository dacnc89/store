namespace :createdb do
  ...
  task create_products: :environment do
    ...
    (1..50).each do |num|
      ...
      product = Product.create(name: "Ipad IPDJKE  #{num} Product #{num}'s name", image_file: "https://photo2.tinhte.vn/data/attachment-files/2017/07/4081488_Tinhte-iPad_Mini-1.jpg", description: "Description for product ipad #{num}", available_quantity: 100, brand_id: apple.id, category_id: ipad.id, company_id: zimean.id, price: num+1.0)
       if product.name.include?(" ")
        product.name = product.name.split(" ").join("-")
        product.save
      else
